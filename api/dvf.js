export default async function handler(req, res) {
  ...
  try {
    ...
    // Étape 2 — API DVF Etalab (seule URL qui marche)
    const dvfRes = await fetch(
      `https://api.dvf.etalab.gouv.fr/api/geoapi/mutations/?code_commune=${codeInsee}&type_local=Appartement&fields=valeur_fonciere,surface_reelle_bati,date_mutation&ordering=-date_mutation&page_size=40`,
      { headers: { 'Accept': 'application/json' } }
    )

    if (!dvfRes.ok) {
      return res.status(200).json({
        error: 'API DVF indisponible (statut ' + dvfRes.status + ')',
        dvf: null,
    ...
    if (records.length === 0) {
      return res.status(200).json({
        error: 'Aucune vente trouvee pour cette commune (' + codeInsee + ')',
        dvf: null,
        geo: { lat, lng, ville, codePostal, codeInsee }
      })
    }
