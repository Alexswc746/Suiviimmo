export default async function handler(req, res) {
  ...
  try {
    ...
    const codeDept = feature.properties.context?.split(',')[0]?.trim() || codeInsee.substring(0, 2)

    const surfNum = surface ? parseFloat(surface) : null

    // Étape 2 — Recherche par proximité GPS (paramètre lng correct)
    const nearbyUrl = `https://immoapi.app/v1/mutations/nearby?lat=${lat}&lng=${lng}&radius=2000&type_local=Appartement&per_page=40`
    ...
    if (dvfRes.ok) {
      ...
      console.log('Nearby OK — total:', dvfData.total, 'mutations:', (dvfData.mutations||[]).length)
      mutations = dvfData.mutations || []
    } else {
      const errText = await dvfRes.text()
      console.log('Nearby error:', dvfRes.status, errText.substring(0, 200))
    }

    // Fallback — recherche par département si pas assez de résultats
    if (mutations.length < 3) {
      const deptUrl = `https://immoapi.app/v1/mutations?code_departement=${codeDept}&type_local=Appartement&per_page=40`
      ...
      if (deptRes.ok) {
        ...
        console.log('Dept OK — total:', deptData.total, 'mutations:', (deptData.mutations||[]).length)
        mutations = deptData.mutations || []
      } else {
        const errText = await deptRes.text()
        console.log('Dept error:', deptRes.status, errText.substring(0, 200))
      }
    }
    ...
    const normalized = mutations
      .map(r => ({
        prix: parseFloat(r.valeur_fonciere || 0),
